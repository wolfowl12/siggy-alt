        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-7 scrollbar-hide pb-36">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center opacity-30 text-center py-20">
              <p className="text-[10px] uppercase tracking-[0.6em] text-[#ffea00]">Awaiting Ritual Protocol</p>
            </div>
          )}
          {messages.map((m, i) => (
          ...
          {loading && <div className="text-[9px] text-[#00ff41] animate-pulse uppercase tracking-[0.3em]">Processing...</div>}
        </div>

        <div className="p-4 absolute bottom-0 w-full bg-gradient-to-t from-black to-transparent">
          <div className="bg-[#08080a] border border-[#00ff41]/20 rounded-lg flex p-2 gap-2 focus-within:border-[#ffea00]/50 shadow-[0_0_20px_rgba(0,255,65,0.05)]">
            <input 
            ...
            />
            <button onClick={sendMessage} className="bg-[#ff003c] text-white font-black text-[9px] px-5 rounded-md uppercase hover:bg-[#ff1a4d] transition-all">
              Execute
            </button>
          </div>
        </div>
